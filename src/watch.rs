...
impl Watcher {
	pub fn new(url: String, ignore_patterns: &[String], notifier: mpsc::Sender<NotificationEvent>) -> Result<Self, DominionError> {
		...
		match self.fetch().await {
			Ok(content) => {
				...
				if let Some(prev) = &self.previous {
					if current_hash == self.previous_hash {
						...
						self.notifier.send(NotificationEvent::Changed {
							url: self.url.clone(),
							old: prev.to_string(),
							new: current.clone(),
						}).await?;
			...
			Err(err) => {
				if !self.last_failed {
					...
					self.notifier.send(NotificationEvent::Failed {
						url: self.url.clone(),
						reason: format!("{}", err),
					}).await?;
				}
